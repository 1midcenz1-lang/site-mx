(function () {
  ...
  document.addEventListener("click", async (evt) => {
    ...
    if (rejectBtn) {
      ...
      const res = await fetch(`/khnowledge-mx/api/requests/${rid}/reject`, { method: "POST", body: fd });
      const data = await res.json();
    ...
    if (fakeBtn) {
      ...
      const fd = new FormData();
      fd.append("reason", reason);
      const res = await fetch(`/khnowledge-mx/api/requests/${rid}/fake`, { method: "POST", body: fd });
      const data = await res.json();
    ...
    if (reportBanBtn) {
      ...
      const res = await fetch("/khnowledge-mx/api/device-ban", { method: "POST", body: fd });
      const data = await res.json();
    ...
    if (reportDeleteBtn) {
      ...
      const res = await fetch(`/khnowledge-mx/api/reports/${rid}/delete`, { method: "POST" });
      const data = await res.json();
  ...
  if (settingsForm) {
    ...
    settingsForm.addEventListener("submit", async (e) => {
      ...
      const res = await fetch("/khnowledge-mx/api/settings", { method: "POST", body: fd });
      const data = await res.json();
      if (!res.ok || !data.ok) {
  ...
  if (categoryForm) {
    categoryForm.addEventListener("submit", async (e) => {
      ...
      const res = await fetch("/khnowledge-mx/api/categories", { method: "POST", body: fd });
      const data = await res.json();
      if (!res.ok || !data.ok) {
  ...
  if (backupAllBtn) {
    backupAllBtn.addEventListener("click", () => {
      window.location.href = "/khnowledge-mx/api/backup-all";
    });
  }
  ...
  document.querySelectorAll(".save-category-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      const res = await fetch(`/khnowledge-mx/api/categories/${id}/update`, { method: "POST", body: fd });
      const data = await res.json();
      if (!res.ok || !data.ok) {
  ...
  document.querySelectorAll(".delete-category-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      const id = btn.dataset.categoryId;
      const res = await fetch(`/khnowledge-mx/api/categories/${id}/delete`, { method: "POST" });
      const data = await res.json();
      if (!res.ok || !data.ok) {
  ...
  document.querySelectorAll(".delete-video-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      const res = await fetch(`/khnowledge-mx/api/videos/${id}/delete`, { method: "POST" });
      const data = await res.json();
      if (!res.ok || !data.ok) {
  ...
  document.querySelectorAll(".delete-testimonial-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      const res = await fetch(`/khnowledge-mx/api/testimonials/${id}/delete`, { method: "POST" });
      const data = await res.json();
      if (!res.ok || !data.ok) {
  ...
  document.querySelectorAll(".approve-testimonial-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const id = btn.dataset.testimonialId;
      const res = await fetch(`/khnowledge-mx/api/testimonials/${id}/approve`, { method: "POST" });
      const data = await res.json();
      if (!res.ok || !data.ok) {
  ...
  document.querySelectorAll(".reject-testimonial-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const id = btn.dataset.testimonialId;
      const res = await fetch(`/khnowledge-mx/api/testimonials/${id}/reject`, { method: "POST" });
      const data = await res.json();
      if (!res.ok || !data.ok) {
  ...
  if (videoForm) {
    videoForm.addEventListener("submit", async (e) => {
      ...
      xhr.open("POST", "/khnowledge-mx/api/videos");
  ...
  function bindApproveForm(form) {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      const res = await fetch(`/khnowledge-mx/api/requests/${rid}/approve`, { method: "POST", body: fd });
      const data = await res.json();
      if (!res.ok || !data.ok) {
  ...
  document.querySelectorAll(".ban-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      const res = await fetch(`/khnowledge-mx/api/visitors/${id}/ban`, { method: "POST" });
      const data = await res.json();
      if (!res.ok || !data.ok) {
  ...
  document.querySelectorAll(".unban-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const id = btn.dataset.visitorId;
      const res = await fetch(`/khnowledge-mx/api/visitors/${id}/unban`, { method: "POST" });
      const data = await res.json();
      if (!res.ok || !data.ok) {
  ...
  document.querySelectorAll(".reply-form").forEach((form) => {
    form.addEventListener("submit", async (e) => {
      ...
      const fd = new FormData(form);
      const res = await fetch(`/khnowledge-mx/api/reports/${rid}/reply`, { method: "POST", body: fd });
      const data = await res.json();
      if (!res.ok || !data.ok) {
  ...
  document.querySelectorAll(".delete-admin-msg-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      const res = await fetch(`/khnowledge-mx/api/reports/${rid}/messages/${msgId}/delete`, { method: "POST" });
      const data = await res.json();
  ...
  async function refreshLiveFeed() {
    ...
    try {
      const res = await fetch("/khnowledge-mx/api/live-feed");
      const data = await res.json();
      ...
      if (purchaseRowsBody && Array.isArray(data.purchases)) {
        ...
        purchaseRowsBody.innerHTML = data.purchases.map((r) => `
          <tr data-request-id="${r.id}" data-requested-category="${escapeHtml(r.requested_category)}" data-user-id="${escapeHtml(r.user_id)}">
            ...
            <td><a class="receipt-open-link" data-receipt-url="/khnowledge-mx/receipt/${encodeURI(r.receipt_path || "")}?rid=${encodeURIComponent(r.id || "")}" href="/khnowledge-mx/receipt/${encodeURI(r.receipt_path || "")}?rid=${encodeURIComponent(r.id || "")}">مشاهده</a></td>
      ...
      if (reportRowsBody && Array.isArray(data.reports)) {
        reportRowsBody.innerHTML = data.reports.map((rp) => `
          <tr>
            ...
            <td><a class="btn small" href="/khnowledge-mx/reports/${rp.id}">باز کردن تیکت</a> <button class="btn small btn-danger report-ban-btn" data-device-id="${escapeHtml(rp.device_id)}" type="button">بن</button> <button class="btn small btn-ghost report-delete-btn" data-report-id="${escapeHtml(rp.id)}" type="button">حذف</button></td>
          </tr>
        `).join("");
      }
    } catch (_err) {}
  ...
  if (statNodes.length > 0) {
    ...
    refreshLiveStats = async () => {
      try {
        const res = await fetch("/khnowledge-mx/api/live-stats");
        const data = await res.json();
        if (!res.ok || !data.ok) return;
